...
class MatrixViewer:
    def __init__(self, parent, matrixdata):
        self.matrixdatamain = matrixdata
        ...
        for i in range(self.tableamount):
            ...
            self.column_entries.append(columnsname)
            self.row_entries.append(rowsname)
        ...
        self.buttonfix = ttk.Button(self.root, text="Перерахунок", style="Big.TButton", command = self.collect_user_data)
    ...
    def destroy_and_create(self):

        backend = BackEnd(self.parent, self.root, self.matrixdatamain, self.matrixdata, self.column_entries, self.row_entries)
